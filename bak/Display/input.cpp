...
void pressed_menu()
{
  switch (LCD_Menu)
  {
  case M_SCR_MAIN_VIEW:
    display->LCD_display_main_view();
    break;
  case M_LIST_PROGRAMS:
    display->LCD_display_programs();
    break;
  case M_QUICK_PROGRAM:
    display->LCD_Display_quick_program(0, 0);
    break;
  case M_INFORMATIONS:
    display->LCD_Display_info();
    break;
  case M_PREFERENCES:
    display->LCD_Display_prefs();
    break;
  case M_CONNECT_WIFI:
    display->LCD_Reconect_WiFi();
    break;
  case M_ABOUT:
    display->LCD_Display_about();
    break;
  case M_RESTART:
    display->Restart_ESP();
    break;
...
void button_Short_Press()
{
  ...
  if (LCD_State == SCR_MENU)
    ...
    display->LCD_display_mainv3(0, 2);
  else if (LCD_State == SCR_PROGRAM_LIST)
    display->LCD_Display_program_summary(0, 0);
  else if (LCD_State == SCR_PROGRAM_SHOW)
    display->LCD_Display_program_summary(0, 2);
  else if (LCD_State == SCR_PROGRAM_DELETE)
    display->LCD_Display_program_delete(0, 1);
  else if (LCD_State == SCR_PROGRAM_FULL)
    display->LCD_Display_program_summary(0, 1);
  else if (LCD_State == SCR_QUICK_PROGRAM)
    display->LCD_Display_quick_program(0, 2);
  else
    display->LCD_display_menu(); // if pressed something else - go back to menu
}
...
void button_Long_Press()
{
  ...
  if (LCD_State == SCR_MENU)
  { // we are in menu - switch to main screen
    LCD_State = SCR_MAIN_VIEW;
    display->LCD_display_main_view();
    return;
  }
  else if (LCD_State == SCR_PROGRAM_SHOW)
  { // if we are showing program - go to program list
    LCD_State = SCR_PROGRAM_LIST;
    display->LCD_display_programs(); // LCD_Program is global
    return;
  }
  else
  {                       // If we are in MAIN screen or Program list or in unknown area to to -> menu
    LCD_State = SCR_MENU; // switching to menu
    display->LCD_display_menu();
    return;
  }
}
...
void Rotate()
{
  ...
  {
    if (LCD_Main == MAIN_VIEW3 && Program_run_size)
    { // if we are on third screen and program is loaded - let the screen 3 know about turn
      display->LCD_display_mainv3(encoderValue, 1);
      return;
    }
    if (encoderValue < 0)
    { // other screens just go...
      if (LCD_Main > MAIN_VIEW1)
        LCD_Main = (LCD_MAIN_View_enum)((int)LCD_Main - 1);
      display->LCD_display_main_view();
      return;
    }
    else
    {
      if (LCD_Main < MAIN_end - 1)
        LCD_Main = (LCD_MAIN_View_enum)((int)LCD_Main + 1);
      display->LCD_display_main_view();
      return;
    }
  ...
  else if (LCD_State == SCR_MENU)
  {
    ...
    if (encoderValue < 0)
    {
      if (LCD_Menu > M_SCR_MAIN_VIEW)
        LCD_Menu = (LCD_SCR_MENU_Item_enum)((int)LCD_Menu - 1);
    }
    else
    {
      if (LCD_Menu < M_END - 1)
  ...
  else if (LCD_State == SCR_PROGRAM_LIST)
  {
    ...
    display->LCD_Display_program_summary(encoderValue, 1);
  else if (LCD_State == SCR_PROGRAM_DELETE)
    display->LCD_Display_program_delete(encoderValue, 0);
  else if (LCD_State == SCR_PROGRAM_FULL)
    display->LCD_Display_program_full(encoderValue);
  else if (LCD_State == SCR_PREFERENCES)
    display->LCD_Display_prefs(encoderValue);
  else if (LCD_State == SCR_QUICK_PROGRAM)
    display->LCD_Display_quick_program(encoderValue, 1);
}
...
void handleInterrupt()
{
  ...
  {                                         // Those two events can be simultaneous - but this is also ok, usually user does not press and turn
