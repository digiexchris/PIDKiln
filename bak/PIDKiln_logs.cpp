...
extern uint16_t Energy_Wattage;
...
uint8_t Load_LOGS_Dir()
{
  ...
  Logs_DIR_size = 0;
  dir.rewindDirectory();
  while ((file = dir.openNextFile()) && Logs_DIR_size <= count)
  { // now we do acctual loading into memory
    char tmp[32];
    uint8_t len2;
...
void dbgLog(uint16_t pri, const char *fmt, ...)
{
  ...
  if (!Prefs[PRF_DBG_SERIAL].value.uint8 && !Prefs[PRF_DBG_SYSLOG].value.uint8)
    return; // don't waste time and resources if logging off
