...
byte add_program_line(String &linia)
{
  ...
  if (prg[0] > Prefs[PRF_MAX_TEMP].value.uint16)
    return 4; // check if we do not exceed max temperature
  ...
  if (prg = SPIFFS.open(file_path, "r"))
  {
    ...
    while (prg.available())
    {
      ...
      else
      {
        ...
        if (line.length() > 15)
          return Cleanup_program(2); // program line too long
...
uint8_t Load_programs_dir()
{
  ...
  Programs_DIR_size = 0;
  dir.rewindDirectory();
  while ((file = dir.openNextFile()) && Programs_DIR_size <= count)
  { // now we do actual loading into memory
    char tmp[32];
    uint8_t len2;
  ...
  if (Programs_DIR_size)
    Programs_DIR[0].sel = 1; // make first program selected if we have at least one
  return 0;
}
...
void Update_program_step(uint8_t sstep, uint16_t stemp, uint16_t stime, uint16_t sdwell)
{
  if (Program_run_size <= sstep)
    if (Program_run_size == sstep)
    { // we are out of the program - but this is just NEXT step, we can add
      Program_run_size++;
      Program_run = (PROGRAM *)REALLOC(Program_run, sizeof(PROGRAM) * Program_run_size);
    }
    else
      return; // we are out of the program - we can edit it
...
void rotate_selected_program(int dir)
{
  ...
  else if (dir > 0 && a < (Programs_DIR_size - 1))
  {                          // if we are going UP and there is more programs - do it, if it's the end of program list, do nothing
    Programs_DIR[a].sel = 0; // delete old selection
    Programs_DIR[a + 1].sel = 1;
  }
}
...
//
void Program_recalculate_ETA(bool dwell)
{
  ...
  {
    if (!dwell)
      Program_run_end += Program_run[a].togo * 60;
    else
      dwell = false; // If we start counting with dwell=true - next step shoud start as a normal step, so dwell=fasle;
...
//
void Program_calculate_steps(bool prg_start)
{
  ...
  if (prg_start)
  { // starting program - do some startup stuff
    Program_run_step = -1;
    cnt1 = 0;
  ...
  if (cnt2 > 10)
  {
    ...
    if (Program_run_state == PR_PAUSED)
    { // if we are paused - calculate new ETA end exit
    ...
    { // check if we are in threshold window and there is temperature set already - if not, pause
      ...
      else if (Program_run_state == PR_THRESHOLD)
        Program_run_state = PR_RUNNING;
    }
    ...
    if (time(NULL) > next_step_end)
    {
      ...
      {                     // we have finished full step togo+dwell (or this is first step)
        ...
        if (Program_run_step >= Program_run_size)
        { // we have passed the last step - end program
          END_Program();
          return;
        }
...
void START_Program()
{
  ...
  if (Program_run_state == PR_NONE || Program_run_state == PR_PAUSED)
    return; // check if program is ready (we can start also from ended,failed etc,) and not paused
