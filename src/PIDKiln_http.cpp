...
#include <SPIFFS.h>
#include "PIDKiln.h"
#include "PIDKiln_http.h"
#include "PIDKiln_logs.h"
#include "PIDKiln_program.h"
#include "PIDKiln_main.h"
#include "PIDKiln_prefs.h"
...
String Preferences_parser(const String &var)
{
  ...
  if (var == "WiFi_SSID")
    return String(Prefs[PRF_WIFI_SSID].value.str);
  else if (var == "WiFi_Password")
    return String(Prefs[PRF_WIFI_PASS].value.str);
  else if (var == "WiFi_Mode0" && Prefs[PRF_WIFI_MODE].value.uint8 == 0)
    return "checked";
  else if (var == "WiFi_Mode1" && Prefs[PRF_WIFI_MODE].value.uint8 == 1)
    return "checked";
  else if (var == "WiFi_Mode2" && Prefs[PRF_WIFI_MODE].value.uint8 == 2)
    return "checked";
  else if (var == "WiFi_Mode3" && Prefs[PRF_WIFI_MODE].value.uint8 == 3)
    return "checked";
  else if (var == "WiFi_Retry_cnt")
    return String(Prefs[PRF_WIFI_RETRY_CNT].value.uint8);

  else if (var == "HTTP_Local_JS0" && Prefs[PRF_HTTP_JS_LOCAL].value.uint8 == 0)
    return "checked";
  else if (var == "HTTP_Local_JS1" && Prefs[PRF_HTTP_JS_LOCAL].value.uint8 == 1)
    return "checked";

  else if (var == "Auth_Username")
    return String(Prefs[PRF_AUTH_USER].value.str);
  else if (var == "Auth_Password")
    return String(Prefs[PRF_AUTH_PASS].value.str);

  else if (var == "NTP_Server1")
    return String(Prefs[PRF_NTPSERVER1].value.str);
  else if (var == "NTP_Server2")
    return String(Prefs[PRF_NTPSERVER2].value.str);
  else if (var == "NTP_Server3")
    return String(Prefs[PRF_NTPSERVER3].value.str);
  else if (var == "GMT_Offset_sec")
    return String(Prefs[PRF_GMT_OFFSET].value.int16);
  else if (var == "Daylight_Offset_sec")
    return String(Prefs[PRF_DAYLIGHT_OFFSET].value.int16);
  else if (var == "Initial_Date")
    return String(Prefs[PRF_INIT_DATE].value.str);
  else if (var == "Initial_Time")
    return String(Prefs[PRF_INIT_TIME].value.str);

  else if (var == "MIN_Temperature")
    return String(Prefs[PRF_MIN_TEMP].value.uint8);
  else if (var == "MAX_Temperature")
    return String(Prefs[PRF_MAX_TEMP].value.uint16);
  else if (var == "MAX_Housing_Temperature")
    return String(Prefs[PRF_MAX_HOUSING_TEMP].value.uint16);
  else if (var == "Alarm_Timeout")
    return String(Prefs[PRF_ALARM_TIMEOUT].value.uint16);
  else if (var == "Thermal_Runaway")
    return String(Prefs[PRF_THERMAL_RUN].value.uint16);
  else if (var == "MAX31855_Error_Grace_Count")
    return String(Prefs[PRF_ERROR_GRACE_COUNT].value.uint8);

  else if (var == "PID_Window")
    return String(Prefs[PRF_PID_WINDOW].value.uint16);
  else if (var == "PID_Kp")
    return String(Prefs[PRF_PID_KP].value.vfloat);
  else if (var == "PID_Ki")
    return String(Prefs[PRF_PID_KI].value.vfloat);
  else if (var == "PID_Kd")
    return String(Prefs[PRF_PID_KD].value.vfloat);
  else if (var == "PID_POE0" && Prefs[PRF_PID_POE].value.uint8 == 0)
    return "checked";
  else if (var == "PID_POE1" && Prefs[PRF_PID_POE].value.uint8 == 1)
    return "checked";
  else if (var == "PID_Temp_Threshold")
    return String(Prefs[PRF_PID_TEMP_THRESHOLD].value.int16);

  else if (var == "LOG_Window")
    return String(Prefs[PRF_LOG_WINDOW].value.uint16);
  else if (var == "LOG_Files_Limit")
    return String(Prefs[PRF_LOG_LIMIT].value.uint16);

  else if (var == "DBG_Serial0" && Prefs[PRF_DBG_SERIAL].value.uint8 == 0)
    return "selected";
  else if (var == "DBG_Serial1" && Prefs[PRF_DBG_SERIAL].value.uint8 == 1)
    return "selected";
  else if (var == "DBG_Syslog0" && Prefs[PRF_DBG_SYSLOG].value.uint8 == 0)
    return "selected";
  else if (var == "DBG_Syslog1" && Prefs[PRF_DBG_SYSLOG].value.uint8 == 1)
    return "selected";
  else if (var == "DBG_Syslog_Port")
    return String(Prefs[PRF_SYSLOG_PORT].value.uint16);
  else if (var == "DBG_Syslog_Srv")
    return String(Prefs[PRF_SYSLOG_SRV].value.str);
...
String Debug_ESP32(const String &var)
{
  ...
  else if (var == "FLASH_MODE")
  {
    ...
    if (fMode == FM_QIO)
      mode = "QIO (0) Quad I/O - Fastest";
    else if (fMode == FM_QOUT)
      mode = "QOUT (1) Quad Output - 15% slower than QIO";
    else if (fMode == FM_DIO)
      mode = "DIO (2) Dual I/O - 45% slower than QIO";
    else if (fMode == FM_DOUT)
      mode = "DOUT (3) Dual Output - 50% slower than QIO";
    else if (fMode == FM_FAST_READ)
      mode = "FAST_READ (4)";
    else if (fMode == FM_SLOW_READ)
      mode = "SLOW_READ (5)";
    else
      mode = "Unknown";
...
void Generate_INDEX()
{
  ...
  if (Load_programs_dir())
    return; // can't load directory
  ...
  for (uint16_t a = 0; a < Programs_DIR_size; a++)
  {
    ...
    if (file = SPIFFS.open(tmp.c_str(), "r"))
    {
      ...
      template_str += "<td> Error opening file to read description </td>";
...
String Chart_parser(const String &var)
{
  ...
  if (Program_run_start)
    current_time = Program_run_start;
  else
  ...
  if (var == "CHART_DATA" && Program_run_size > 0)
  {
    ...
    tmp += "{x:'" + String(str) + "'";
    if (Program_start_temp)
      tmp += ",y:" + String(Program_start_temp) + "},";
    else
      tmp += ",y:" + String(kiln_temp) + "},";
  ...
  { // if we have log fie to show - show it on graph, otherwise show just program graph
    if (CSVFile)
...
void handleUpload(void *request, String filename, size_t index, uint8_t *data, size_t len, bool final)
{
  ...
  if (final)
  {
    ...
    { // Everything went fine - commit file
      Generate_INDEX();
      request->redirect("/programs");
    }
  }
...
void POST_Handle_Delete(void *request)
{
  ...
  if (request->hasParam("prog_name", true) && request->hasParam("yes", true))
  {
    ...
    if (p->value().compareTo(String("Yes!")) == 0)
    { // yes user want's to delete program
...
void GET_Handle_Load(void *request)
{
  ...
  if (!request->hasParam("prog_name"))
  { // if no program to load - skip
...
String handleVars(const String &var)
{
  ...
  if (var == "KILN_TEMP")
    return String(kiln_temp);
  else if (var == "SET_TEMP")
    return String(set_temp);
  else if (var == "ENV_TEMP")
    return String(int_temp);
  else if (var == "CASE_TEMP")
  ...
  else if (var == "STEP")
  {
    if (Program_run_state == PR_RUNNING)
    {
      ...
      strcpy(str, Prog_Run_Names[Program_run_state]);
  ...
  if (var == "PROGRAM_STATUS")
    return String(Program_run_state);
  else
    return String(" ");
}
