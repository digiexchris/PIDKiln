...
boolean Change_prefs_value(String item, String value)
{
  ...
  {
    if (item.equalsIgnoreCase(String(PrefsName[a])))
    { // we have found an matching prefs value
...
void Load_prefs()
{
  ...
  if (prf = SPIFFS.open(PREFS_FILE, "r"))
    while (prf.available())
    {
      ...
      if (line.startsWith("#"))
        continue; // skip every comment line
      if (pos = line.indexOf("#"))
      {
      ...
      if ((pos = line.indexOf("=")) > 2)
      { // we need to have = sign - if not, ignore
