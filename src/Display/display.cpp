...
boolean Display::return_LCD_string(char *msg, char *rest, int mod, uint16_t screen_w = SCREEN_W)
{
    if (u8g2 == nullptr)
    {
        return false; // Ensure u8g2 is initialized
    }
    uint16_t chh, lnw;
...
//
void Display::load_msg(char msg[MAX_CHARS_PL])
{
    if (u8g2 == nullptr)
    {
...
//
void Display::DrawVline(uint16_t x, uint16_t y, uint16_t h)
{
    if (u8g2 == nullptr)
    {
        return; // Ensure u8g2 is initialized
...
//
void Display::Draw_Marked_string(char *str, uint8_t pos)
{
    if (u8g2 == nullptr)
    {
        return; // Ensure u8g2 is initialized
...
void Display::DrawMenuEl(char *msg, uint16_t y, uint8_t cnt, uint8_t el, boolean sel)
{
    if (u8g2 == nullptr)
    {
        return; // Ensure u8g2 is initialized
    ...
    if (cnt == el + 1)
        x_w = SCREEN_W - x; // just make sure to draw last square to the end
    else
        x_w++; // some shitty voodoo
    ...
    if (sel)
    {
        ...
        u8g2->drawStr(x_txt, y - 1, msg); // draw text - we should check it's size also..
}
...
//
void Display::LCD_display_mainv3(int dir, byte ctrl)
{
    if (u8g2 == nullptr)
    {
        return; // Ensure u8g2 is initialized
    ...
    if (!Program_run_size)
        return; //  don't go in if no program loaded
    LCD_State = SCR_MAIN_VIEW;
    LCD_Main = MAIN_VIEW3; // just in case...

    if (dir == 0 && ctrl == 0)
        what = 0; // initialize what

    what += dir; // rotate 1/-1

    if (what > 4)
    { // we can rotate out of this screen
        LCD_Main = MAIN_VIEW1;
...
//
void Display::LCD_display_mainv2()
{
    if (u8g2 == nullptr)
    {
        return; // Ensure u8g2 is initialized
    ...
    for (uint8_t a = 0; a < Program_run_size; a++)
    {
        ...
        if (Program_run[a].temp > mxtemp)
            mxtemp = Program_run[a].temp;
    }

    mxx = SCREEN_W - 2; // max X axis - time
    mxy = SCREEN_H - 5; // max Y axis - temperature
    if (ttime)
        scx = (int)((mxx * 100) / ttime); // 1 minute is scx pixels * 100 on graph
    if (mxtemp)
...
//
void Display::LCD_display_mainv1()
{
    if (u8g2 == nullptr)
    {
        return; // Ensure u8g2 is initialized
    ...
    if (!Program_run_size)
        return; //  dont go in if no program loaded
    LCD_State = SCR_MAIN_VIEW;
    ...
    { // just some small animation for running program
        if (cnt == 1)
    ...
    if (Program_run_step > -1)
        ...
        sprintf(msg, "Stp:0/%d r/d Ht:0%% Cse:%.0fC", Program_run_size, case_temp);
...
//
void Display::LCD_display_main_view()
{
    if (u8g2 == nullptr)
    {
        return; // Ensure u8g2 is initialized
    ...
    if (Program_run_size)
    {
        if (LCD_Main == MAIN_VIEW1)
            LCD_display_mainv1(); // if any program loaded and view selected
        else if (LCD_Main == MAIN_VIEW2)
            LCD_display_mainv2();
        else if (LCD_Main == MAIN_VIEW3)
            LCD_display_mainv3();
    }
    else
    {
...
void Display::LCD_display_menu()
{
    ...
    for (int a = 1; a <= SCR_MENU_LINES; a++)
    {
        ...
        if (m_startpos < 0 || m_startpos >= M_END)
            u8g2->drawStr(15, (a * chh) + SCR_MENU_SPACE + center, " "); // just to add some top/bottom unselectable menu positions
        else
        {
...
void Display::LCD_display_programs()
{
    if (u8g2 == nullptr)
    {
        return; // Ensure u8g2 is initialized
    ...
    if ((sel = Find_selected_program()) < 0)
        return;
    ...
    {
        if (Programs_DIR[start_pos].filesize < 999)
...
//
void Display::LCD_Display_program_delete(int dir, boolean pressed)
{
    if (u8g2 == nullptr)
    {
        return; // Ensure u8g2 is initialized
    ...
    if (!dir && !pressed)
        yes = false; // reset to default
    if (pressed)
        if (yes)
        {
            if (Erase_program_file())
            { // removed
...
//
void Display::LCD_Display_program_full(int dir)
{
    if (u8g2 == nullptr)
    {
        return; // Ensure u8g2 is initialized
    ...
    if (dir > 0 && (pos + lines - 1) < Program_size)
        pos++;
    else if (dir < 0 && pos > 0)
        pos--;
...
//
void Display::LCD_Display_program_summary(int dir, byte load_prg)
{
    if (u8g2 == nullptr)
    {
        return; // Ensure u8g2 is initialized
    ...
    {
        if (prog_menu == P_EXIT)
        { // exit - unload program, go to menu
            Cleanup_program();
    ...
    if (SPIFFS.exists(file_path))
    {
        ...
        if (!load_prg)
            if (err = Load_program(nullptr))
            { // loading program - if >0 - failed with error - see description in PIDKiln.h
        ...
        if (load_prg == 2 && prog_menu == P_DELETE)
            LCD_Display_program_delete();
        else
            u8g2->sendBuffer();
    }
}
...
//
void Display::LCD_Display_quick_program(int dir, byte pos)
{
    if (u8g2 == nullptr)
    {
        return; // Ensure u8g2 is initialized
    ...
    if (pos == 0 && dir == 0)
    { // if we are here for the first time (not turn)
        what = ok = 0;
        xm = ym = 0;
        if (kiln_temp > 0)
            qp[0] = kiln_temp; // target temperature
        else
            qp[0] = 20;
        qp[1] = 10; // time to go
        qp[2] = 10; // dwell time
    ...
    {
        if (what == 12)
        { // if button pressed on cancel
            LCD_display_menu();
            return;
        }
        else if (what == 11)
        { // load program end exit
    ...
    { // temperature
        if (dir > 0)
            qp[0] += pow(10, what);
        else if (dir < 0 && pow(10, what) < qp[0])
            qp[0] -= pow(10, what);
    }
    else if (what < 7)
    { // time
        if (dir > 0)
            qp[1] += pow(10, what - 4);
        else if (dir < 0 && pow(10, what - 4) < qp[1])
            qp[1] -= pow(10, what - 4);
    }
    else if (what < 10)
    { // dwell
        if (dir > 0)
            qp[2] += pow(10, what - 7);
        else if (dir < 0 && pow(10, what - 7) < qp[2])
            qp[2] -= pow(10, what - 7);
    }
    else
    ...
    if (qp[0] < 1)
        qp[0] = 1;
    else if (qp[0] > Prefs[PRF_MAX_TEMP].value.uint16)
        qp[0] = Prefs[PRF_MAX_TEMP].value.uint16;
    ...
    if (what < 4)
    { // if we are drawing detailed temperature
        u8g2->setCursor(xm, y += chh);
        Draw_Marked_string(msg, 3 - what);
    }
    else
        u8g2->drawStr(xm, y += chh, msg);

    sprintf(msg, "%03d min.", qp[1]);
    if (what > 3 && what < 7)
    { // if we are drawing detailed time
        u8g2->setCursor(xm, y += chh);
...
void Display::LCD_Display_info()
{
    ...
    else if (WiFi.getMode() == WIFI_STA)
    {
        ...
        u8g2->drawStr(x, y += chh, msg);
        if (WiFi.isConnected())
            sprintf(msg, "WiFi status: connected");
        else
            sprintf(msg, "WiFi status: disconnected");
        u8g2->drawStr(x, y += chh, msg);
    }
    ...
    if (getLocalTime(&timeinfo))
        strftime(msg, MAX_CHARS_PL, "%F %T", &timeinfo);
    else
        sprintf(msg, "Failed to acquire time");
    u8g2->drawStr(x, y += chh, msg);
...
//
void Display::LCD_Display_prefs(int dir)
{
    if (u8g2 == nullptr)
    {
        return; // Ensure u8g2 is initialized
    ...
    for (int a = pos; a <= PRF_end && (a - pos) < lines && y < SCREEN_H - 5; a++)
    {
        if (Prefs[a].type == STRING)
            ...
            sprintf(msg, "%s = %d", PrefsName[a], Prefs[a].value.str);
        if (return_LCD_string(msg, rest, -4))
        {
            u8g2->drawStr(x, y += chh, msg);
            y += chh;
            if (y < SCREEN_H - 4)
                u8g2->drawStr(x, y, rest);
        }
        else
            u8g2->drawStr(x, y += chh, msg);
    }
...
//
void Display::LCD_Display_about()
{
    if (u8g2 == nullptr)
    {
        return; // Ensure u8g2 is initialized
...
//
void Display::Restart_ESP()
{
    if (u8g2 == nullptr)
    {
        ESP.restart(); // early abort if u8g2 is not initialized
        return;
    }
    u8g2->clearBuffer();
    u8g2->setFont(FONT8);
...
//
void Display::LCD_Reconect_WiFi()
{
    if (u8g2 == nullptr)
    {
        return; // Ensure u8g2 is initialized
...
**
*/
void Display::Display_Start(void)
{
    if (u8g2 == nullptr)
    {
        return; // Ensure u8g2 is initialized
...
void Display::WriteScreenshot(void out(const char *s)) {
