...
boolean Start_WiFi_CLIENT()
{
  ...
  if (!Prefs[PRF_WIFI_MODE].value.uint8)
    return 1; // if WiFi disabled
  if (strlen(Prefs[PRF_WIFI_SSID].value.str) < 1 || strlen(Prefs[PRF_WIFI_PASS].value.str) < 1)
    return 1; // missing SSI or password
...
boolean Setup_WiFi()
{
  ...
  if (Prefs[PRF_WIFI_MODE].value.uint8)
  { // we have WiFi enabled in config - start it
    int err = 0;

    if (Prefs[PRF_WIFI_MODE].value.uint8 == 1 || Prefs[PRF_WIFI_MODE].value.uint8 == 2)
    { // 1 - tries as client if failed, be AP; 2 - just try as client
      ...
      else if (Prefs[PRF_WIFI_MODE].value.uint8 == 2)
        return err; // not connected and won't try be AP
    }
    ...
    if (!err)
    { // WiFi enabled in client mode (connected to AP - perhaps with Internet - try it)
      SETUP_WebServer();
      return 0;
    }
    return err;
